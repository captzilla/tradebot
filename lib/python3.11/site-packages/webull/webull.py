...
class webull :
    ...
    def __init__(self, region_code=None) :
        ...
        self._headers = {
            ...
            'hl': 'en',
            'os': 'web',
            'osv': 'Mozilla/5.0 (X11; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/117.0',
            'app': 'global',
            'appid': 'wb_web_app',
            'ver': '3.40.11',
            'lzone': 'dc_core1',
        ...
        self._paper_acct_id =''
        self._access_token = ''
        #self._account_id = '' is this just acct_id ?
        self._trade_token = ''
        self._trade_pin = ''
        self._refresh_token = ''
        self._token_expire = ''
        self._usr = ''
        self._pwd = ''
        #uuid is random declared in buld req headers
        #did is declared in set_did
    ...
    def build_req_headers(self, include_trade_token=True, include_time=False, include_zone_var=True):
        '''
    ...
    def api_login(self, access_token='', refresh_token='', token_expire='', uuid='', mfa=''):
        ...
        self._uuid = uuid
        self._account_id = self.get_account_id(acctid)
    ...
    def refresh_login(self, save_token=False, token_path=None):
        ...
        if 'accessToken' in result and result['accessToken'] != '' and result['refreshToken'] != '' and result['tokenExpireTime'] != '':
            ...
            self._refresh_token = result['refreshToken']
            self._token_expire = result['tokenExpireTime']
            self._acct_id = self.get_account_id()
            if save_token:
    ...
    def get_account_id(self):
        ...
        response = requests.get(self._urls.account_id(self._acct_id), headers=headers, timeout=self.timeout)
        
        result = response.json()
        #print(result) #prints out all the summary json
             
        sec_account_id = result['accountSummaryVO']['secAccountId']
        #print(f'secAccountId: {sec_account_id}')
        return sec_account_id
     
        '''if 'accountSummaryVO' in result and len(result['accountSummaryVO']) > 0 :
        self._account_id = str(result['accountSummaryVO'].get('secAccountId'))
        print(self._acct_id)
        return self._acct_id
        #else:
        #return None  this block of code gets the acctid and passes it into self._acct_id but unecessary if I already declare it unless
        # I want to do this dynamically and remove the acctid from config and in autowebull'''
    ...
    def get_account(self):
        ...
        '''
        headers = self.build_req_headers()
        response = requests.get(self._urls.account(self._acct_id), headers=headers, timeout=self.timeout)
        result = response.json()
         # Extracting the required info
        account_summary = result.get('accountSummaryVO', {})
        asset_ratio_list = account_summary.get('assetRatioList',[])
        net_liquidation_value = account_summary.get('netLiquidationValue')
        total_market_value = account_summary.get('totalMarketValue')
        total_cash_value = account_summary.get('totalCashValue')
        day_buying_power = account_summary.get('dayBuyingPower')
        unrealized_profit_loss = account_summary.get('unrealizedProfitLoss')
        unrealized_profit_loss_rate = account_summary.get('unrealizedProfitLossRate')
        if unrealized_profit_loss_rate is not None:
            unrealized_profit_loss_rate = f'{float(unrealized_profit_loss_rate) * 100:.2f}%'

        settled_funds = account_summary.get('settledFunds')
        unsettled_funds = account_summary.get('unsettledFunds')
        
        # Printing out the extracted info
        print(f'Net Liquidation Value: {net_liquidation_value}\n'
            f'Total Market Value: {total_market_value}\n'
            f'Total Cash Value: {total_cash_value}\n'
            f'Day Buying Power: {day_buying_power}\n'
            f'Unrealized Profit Loss: {unrealized_profit_loss}\n'
            f'Unrealized Profit Loss Rate: {unrealized_profit_loss_rate}\n'
            f'Settled Funds: {settled_funds}\n'
            f'Unsettled Funds: {unsettled_funds}')

        for item in asset_ratio_list:
            name = item.get('name')
            value = item.get('value')
            print(f'Asset Ratio - {name}: {value}')
    ...
    def openorder(self):
        headers = self.build_req_headers()
        response = requests.get(self._urls.openorder(self._acct_id), headers=headers, timeout=self.timeout)
        result = response.json()
        print(result)
        return result
    ...
    def get_positions(self):
        ...
        '''
        positionsdata = self.get_account()
        # Extract the 'positions' data
        asset_summary = positionsdata.get('assetSummaryVO', {})
        capital = asset_summary.get('capital', [])
        positions = asset_summary.get('positions', [])
        #print(positions) 
        #return positions  this is for the full json of position for debugging usually or if I want to print the whole positions list

        extracted_positions = []

        for position in positions:
            extracted_position = {
                'Dis Symbol': position.get('items')[0].get('ticker').get('disSymbol'),
                'Action': position.get('items')[0].get('action'),
                'Quantity': position.get('quantity'),
                'Option Type': position.get('items')[0].get('optionType'),
                'Option Expire Date': position.get('items')[0].get('optionExpireDate'),
                'Total Cost': position.get('totalCost'),
                'Market Value': position.get('marketValue'),
                'Unrealized Profit Loss': position.get('unrealizedProfitLoss'),
                'Unrealized Profit Loss Rate': f"{float(position.get('unrealizedProfitLossRate')) * 100:.2f}%",
                'Last Price': position.get('items')[0].get('lastPrice'),
                'Belong Trade Price': position.get('items')[0].get('belongTradePrice'),
                'Cost Price': position.get('items')[0].get('costPrice'),
                'Remain Day': position.get('items')[0].get('remainDay')
            }
    ...
    def get_history_orders(self, status='All', count=30):
        '''
        ...
        data = {
            "dateType": "ORDER",
            "pageSize": count,
            "startTimeStr": "2023-10-01",
            "endTimeStr": "2023-10-02",
            "action": None,
            "lastCreateTime0": 0,
            "secAccountId": self._acct_id,
            "status": status
        }
        headers = self.build_req_headers(include_trade_token=True, include_time=True)
        response = requests.post(self._urls.history(self._acct_id), json=data, headers=headers)           
    ...
    def get_option_quote(self, stock=None, tId=None, optionId=None):
        ...
        r = requests.get(self._urls.option_quotes(), params=params, headers=headers, timeout=self.timeout).json()
        #print (r) debugging
        return r
    ...
    def get_options_expiration_dates(self, stock=None, count=-1):
        ...
        res = requests.post(self._urls.options_exp_date_new(), json=data, headers=headers, timeout=self.timeout).json()
        '''print("Keys in the JSON response:") this checks for keys in table
        for key in res.keys():
            print(key)'''
        r_data = []
        for entry in res['expireDateList'] :
            print(f'entry from expireDateList: {entry}')
        ...
        #print(f'List of expiration dates (r_data): {r_data}')
        return r_data

    def get_options(self, stock=None, count=-1, direction='all', expireDate=None):
        '''
    ...
    def place_option_order(self, optionId=None, lmtPrice=None, stpPrice=None, action=None, orderType='LMT', enforce='GTC', quant=0):
        ...
        headers = self.build_req_headers(include_trade_token=True, include_time=False)
        data = {
            'orderType': orderType,
            'serialId': str(uuid.uuid4()),
            'timeInForce': enforce,
            'orders': [{'quantity': int(quant), 'action': action,
             'tickerId': int(optionId), 'tickerType': 'OPTION'}],
        }
        ...
        response = requests.post(self._urls.place_option_order(self._acct_id), json=data, headers=headers, timeout=self.timeout)
...
class paper_webull(webull):
    ...
    def get_account(self, paper_acct_id):
        ''' Get important details of paper account '''
        headers = self.build_req_headers()
        response = requests.get(self._urls.paper_account(self._paper_acct_id), headers=headers, timeout=self.timeout)
        return response.json()
    ...
    def get_account_id(self):
        ...
        if result is not None and len(result) > 0 and 'id' in result[0]:
            id = result[0]['id']
            self._account_id = id
            #print(f"Account ID: {id}")  # Print the ID here
            return id
        else:
            print("None found")
            return None
    ...
    def get_history_orders(self, status='Cancelled', count=30):
        headers = self.build_req_headers(include_trade_token=True, include_time=True)
        response = requests.get(self._urls.paper_orders(self._paper_acct_id, count) + str(status), headers=headers, timeout=self.timeout)
        return response.json()
    ...
    def place_option_order(self, optionId=None, lmtPrice=None, stpPrice=None, action=None, orderType='LMT', enforce='GTC', quant=0):
        ...
        response = requests.post(self._urls.paper_place_option_order(), json=data, headers=headers, timeout=self.timeout)
        #print(response) #for debugging 

        if response.status_code != 200:
            raise Exception('place_option_order failed', response.status_code, response.reason)
        
        # This is the new line you'd add
        if response.status_code == 200:
            print("Successfully placed the paper option order")
