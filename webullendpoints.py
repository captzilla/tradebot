...
class urls :
    ...
    def real_account(self,account_id):
    ...
    def login(self):
        return f'{self.base_userfintech_url}/api/user/v1/login/account/v2'
    ...
    def trade_token(self):
        return f'{self.base_new_trade_url}/api/user/v1/security/login'
...
'''
- `account(account_id)`: `base_trade_url`
- `account_id()`: `base_trade_url`
- `account_activities(account_id)`: `base_ustrade_url`
- `active_gainers_losers(direction, region_code, rank_type, num)`: `base_fintech_gw_url`
- `add_alert()`: `base_userbroker_url`
- `analysis(stock)`: `base_securities_url`
- `analysis_shortinterest(stock)`: `base_securities_url`
- `analysis_institutional_holding(stock)`: `base_securities_url`
- `analysis_etf_holding(stock, has_num, page_size)`: `base_securities_url`
- `analysis_capital_flow(stock, show_hist)`: `base_securities_url`
- `bars(stock, interval='d1', count=1200, timestamp=None)`: `base_fintech_gw_url`
- `bars_crypto(stock)`: `base_fintech_gw_url`
- `cancel_order(account_id)`: `base_ustrade_url`
- `modify_otoco_orders(account_id)`: `base_ustrade_url`
- `cancel_otoco_orders(account_id, combo_id)`: `base_ustrade_url`
- `check_otoco_orders(account_id)`: `base_ustrade_url`
- `place_otoco_orders(account_id)`: `base_ustrade_url`
- `dividends(account_id)`: `base_trade_url`
- `fundamentals(stock)`: `base_securities_url`
- `is_tradable(stock)`: `base_trade_url`
- `list_alerts()`: `base_userbroker_url`
- `login()`: `base_userfintech_url`
- `get_mfa()`: `base_userfintech_url`
- `check_mfa()`: `base_userfintech_url`
- `get_security(username, account_type, region_code, event, time, url=0)`: `base_user_url`
- `next_security(username, account_type, region_code, event, time, url=0)`: `base_user_url`
- `check_security()`: `base_user_url`
- `logout()`: `base_userfintech_url`
- `news(stock, Id, items)`: `base_fintech_gw_url`
- `option_quotes()`: `base_options_gw_url`
- `options(stock)`: `base_options_url`
- `options_exp_date(stock)`: `base_options_url`
- `options_exp_dat_new()`: `base_fintech_gw_url`
- `options_bars(derivativeId)`: `base_options_gw_url`
- `orders(account_id, page_size)`: `base_ustradebroker_url`
- `history(account_id)`: `base_ustrade_url`
- `paper_orders(paper_account_id, page_size)`: `base_paper_url`
- `paper_account(paper_account_id)`: `base_paperfintech_url`
- `paper_account_id()`: `base_paperfintech_url`
- `paper_cancel_order(paper_account_id, order_id)`: `base_paper_url`
- `paper_modify_order(paper_account_id, order_id)`: `base_paper_url`
- `paper_place_order(paper_account_id, stock)`: `base_paper_url`
- `place_option_orders(account_id)`: `base_ustrade_url`
- `place_orders(account_id)`: `base_ustrade_url`
- `modify_order(account_id, order_id)`: `base_ustrade
'''